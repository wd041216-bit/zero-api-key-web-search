#!/usr/bin/env python3
"""Interactive provider setup wizard for zerosieve.
...
from zerosieve.providers import BrightDataProvider, SearxngProvider, WebUnlockerProvider
    ...
    try:
        ...
        if "zone" in error_msg.lower() and "not found" in error_msg.lower():
            return {
                "success": False,
                "error": (
                    f"Zone '{zone}' not found. Create it in Bright Data Dashboard: "
                    f"Proxies -> Add new zone -> SERP API -> name it '{zone}'"
                ),
                "needs_zone": True,
            }
...
def run_interactive_setup(write_env: bool = True) -> None:
    ...
    print("\n" + "=" * 60)
    print("🔧 ZeroSieve — Provider Setup")
    print("=" * 60)
    ...
    if bd_key:
        ...
        if result["success"]:
            ...
            if result.get("needs_zone"):
                ...
                print("     Then re-run this setup wizard.")
    ...
    if bd_key:
        ...
        if result["success"]:
            ...
            print(f"  ❌ {result['error']}")
            if result.get("needs_zone"):
                print("  📝 Create a Web Unlocker zone in Bright Data Dashboard:")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ZeroSieve — Provider Setup Wizard",
        epilog=(
            "Examples:\n"
            "  zs-setup\n"
            "  zs-setup --status\n"
            "  zs-setup --test-brightdata YOUR_API_KEY\n"
            "  zs-setup --test-searxng http://localhost:8080\n"
        ),
    ...
    parser.add_argument(
        ...
        help="Web Unlocker zone name (default: web_unlocker1)",
    )
    ...
    if args.test_brightdata_unlocker:
        ...
        if result["success"]:
            ...
            if result.get("needs_zone"):
                print(
                    f"   Create zone '{args.unlocker_zone}' in Bright Data Dashboard "
                    "-> Proxies -> Add new zone -> Web Unlocker"
                )
        return
