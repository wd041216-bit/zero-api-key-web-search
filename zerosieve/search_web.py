...
from zerosieve.core import (  # noqa: F401
    Answer,
    Source,
    UltimateSearcher,
)
from zerosieve.laya_filter import add_laya_args, laya_backend_from_args, laya_search_kwargs
...
def main():
    ...
    if len(sys.argv) >= 2 and sys.argv[1] in {"providers", "doctor"}:
        provider_parser = argparse.ArgumentParser(
            description="Inspect configured ZeroSieve providers.",
        )
    ...
    parser = argparse.ArgumentParser(
        description="ZeroSieve CLI (package: zerosieve)",
        epilog=(
            "Examples:\n"
    ...
    add_laya_args(parser)

    args = parser.parse_args()

    searcher = UltimateSearcher(laya_backend=laya_backend_from_args(args))
            ...
            try:
                ...
                if parsed_repl.query:
                    answer = searcher.search(
                        ...
                        **kwargs,
                        **laya_search_kwargs(parsed_repl),
                    )
                    print(
    ...
    if args.json:
        ...
        print(f"Providers used: {used} | run `zs-search providers` to inspect optional providers")
        print(f"{'='*60}\n")
