#!/usr/bin/env python3
"""ZeroSieve MCP server.

The canonical MCP command is ``zs-mcp``.
Legacy alias ``cross-validated-search-mcp`` remains available for compatibility.
"""
import asyncio
import logging
import sys
...
from zerosieve.cache import clear_cache, get_cache
from zerosieve.core import UltimateSearcher

# Configure logging
logging.basicConfig(level=logging.INFO, stream=sys.stderr)
logger = logging.getLogger("zerosieve-mcp")

app = Server("zerosieve")
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "engine": {
                        ...
                        "type": "boolean",
                        "description": (
                            "Enable the Laya neural sieve (local decision model, Apache-2.0): "
                            "probabilistic relevance filtering / stance classification in one "
                            "forward pass per item (~35ms GPU). Falls back to the lexical "
                            "heuristic when the laya package or checkpoint is unavailable."
                        ),
                        "default": False
                    },
                    "laya_threshold": {
                        "type": "number",
                        "description": "Probability cutoff for laya filtering/stance (default 0.5).",
                        "default": 0.5
                    },
"required": ["query"]
            }
    ...
    try:
        ...
        if name == "search_web":
            ...
            answer = searcher.search(
                ...
                profile=profile,
                goggles=goggles,
                laya_filter=bool(arguments.get("laya", False)),
                laya_threshold=float(arguments.get("laya_threshold", 0.5)),
                **extra_kwargs,
            )
            ...
            laya_meta = answer.metadata.get("laya_filter") or {}
            if laya_meta.get("enabled"):
                result_text += (
                    f"Laya sieve: kept {laya_meta.get('kept', 0)}, "
                    f"dropped {laya_meta.get('dropped', 0)} "
                    f"(threshold {laya_meta.get('threshold')})\n"
                )
            ...
            verification = searcher.verify_claim(
                ...
                deep=deep,
                max_pages=max_pages,
                verifier="laya" if arguments.get("laya", False) else "heuristic",
                laya_threshold=float(arguments.get("laya_threshold", 0.5)),
            )
            ...
            from zerosieve.provider_setup import _test_brightdata, _test_searxng, _test_web_unlocker
...
async def main():
    """Run the MCP server."""
    logger.info("Starting ZeroSieve MCP Server...")
