...
# ZeroSieve for OpenClaw

This skill gives OpenClaw a practical verification workflow:

- `zs-search` for live search results (7 engines via Bright Data)
- `zs-search providers` for provider discovery
- `zs-browse` for reading pages (auto-unlocks blocked content)
- `zs-verify` for support/conflict classification
- `zs-report` for a citation-ready summary with next steps
- `zs-setup` for interactive provider configuration
...
```bash
zs-search "OpenAI API pricing" --type news --timelimit w
zs-search providers
zs-browse "https://docs.python.org/3/whatsnew/"
zs-verify "Python 3.13 is the latest stable release" --deep --max-pages 2 --json
zs-report "Python 3.13 stable release" --claim "Python 3.13 is the latest stable release" --deep --json
```
...
1. Run `zs-search` for factual or recent questions.
2. Use `zs-browse` on the most relevant source when snippets are not enough.
3. Use `zs-verify` when a concrete claim needs a support/conflict summary.
4. Use `zs-report` when you want a compact evidence package with citations and next steps.
...
```bash
zs-search "AI regulation" --provider brightdata --engine google --region us-en --json
zs-search "AI regulation" --provider brightdata --engine bing --region gb-en --json
zs-search "news" --provider brightdata --engine yandex --region ru-ru --json
```
...
zs-browse "https://protected-site.com/article"

# Force Web Unlocker
zs-browse "https://protected-site.com/article" --use-unlocker always
```
...
- `zs-verify` is heuristic and evidence-aware, not a proof engine.
