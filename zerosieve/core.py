...
from zerosieve.cache import get_cache
from zerosieve.providers import (
    BrightDataProvider,
    DdgsProvider,
...
logger = logging.getLogger("zerosieve")
...
class UltimateSearcher:
    ...
    def _resolve_laya_backend(self) -> "LayaBackend":
        ...
        if self._laya_backend is None:
            ...
            self._laya_backend = LayaBackend()
        return self._laya_backend
    ...
    def search(
        ...
        laya_filter: bool = False,
        laya_threshold: float = 0.5,
        **kwargs,
    ) -> Answer:
        """Search the web and return a structured answer.

        With ``laya_filter=True`` the merged results pass through a Laya
        relevance sieve (one forward pass per result) before returning;
        see :mod:`zerosieve.laya_filter`.
        """
        ...
        laya_metadata: dict = {"enabled": False}
        if laya_filter and validated_results:
            from .laya_filter import LayaUnavailableError, filter_sources

            try:
                validated_results, laya_metadata = filter_sources(
                    query,
                    validated_results,
                    self._resolve_laya_backend(),
                    threshold=laya_threshold,
                )
            except LayaUnavailableError as exc:
                logger.warning("laya filter unavailable, keeping all results: %s", exc)
                laya_metadata = {"enabled": False, "error": str(exc)[:200]}
    ...
    def _classify_via_verifier(
        ...
        """Dispatch source classification to Laya or the lexical heuristic.

        The Laya path keeps the full lexical payload as a comparable sidecar
        and overrides ``classification`` / ``evidence_strength`` from the
        neural stance probabilities blended with quality and freshness.
        """
        ...
        from .laya_filter import LayaUnavailableError, blend_strength, classify_stance

        try:
            laya_classification, laya_evidence = classify_stance(
                claim,
                source,
                self._resolve_laya_backend(),
                threshold=laya_threshold,
            ...
            logger.warning("laya verifier unavailable mid-run, keeping lexical classification: %s", exc)
            evidence["verification_model_note"] = str(exc)[:200]
            return classification, evidence
        evidence.update(laya_evidence)
        stance = laya_evidence.get("laya_stance", {})
        if stance.get("status") != "ok":
            evidence["verification_model_note"] = "laya stance errored; kept lexical classification"
            return classification, evidence
        ...
        evidence["evidence_strength"] = blend_strength(
            stance_prob,
            evidence["source_quality"]["score"],
            evidence["freshness"]["score"],
        )
        return laya_classification, evidence
    ...
    def verify_claim(
        ...
        **kwargs,
        ...
        ``verifier="laya"`` classifies each source's stance with the Laya
        decision model (one forward pass per source) instead of the lexical
        heuristic, falling back to the heuristic when Laya is unavailable.
        """
        start_time = time.time()
        if verifier not in ("heuristic", "laya"):
            raise ValueError(f"unknown verifier: {verifier!r} (expected 'heuristic' or 'laya')")
        use_laya = False
        if verifier == "laya":
            try:
                self._resolve_laya_backend().warmup()
                use_laya = True
            except Exception as exc:
                logger.warning("laya verifier unavailable, using lexical heuristic: %s", exc)
        logger.info("verify_claim: claim=%r deep=%s verifier=%s", claim, deep, "laya" if use_laya else "heuristic")
        ...
        if len(sub_claims) > 1:
            for sub in sub_claims:
                ...
                for source in sub_answer.sources:
                    classification, evidence = self._classify_via_verifier(
                        sub,
                        sub_keywords,
                        source,
                        timelimit=timelimit,
        ...
        return VerificationResult(
            ...
            analysis={
                ...
                "verification_model": {
                    "name": "laya-stance-v1" if use_laya else "evidence-aware-heuristic-v3",
                    "score_breakdown": (
                        {
                            "laya_stance_weight": 0.55,
                            "source_quality_weight": 0.35,
                            "source_freshness_weight": 0.10,
                        }
                        if use_laya
                        else {
                    ...
                    + (
                        [
                            "laya_stance_probabilities_are_not_domain_calibrated; "
                            "refit thresholds on a small labeled set from your domain "
                            "before trusting verdicts at scale",
                            "page-aware rescoring (--deep) still uses the lexical path",
                        ]
                        if use_laya
                        else []
                    ),
                ...
                "semantic_oracle_boundary": {
                    ...
                    "neural_signals": ["laya_stance_probabilities"] if use_laya else [],
                    "note": (
                        "Stance classification uses Laya decision probabilities; no logical "
                        "entailment proof is performed and probabilities are not domain-calibrated."
                        if use_laya
                        else "No natural language inference or entailment is performed. "
    ...
    def evidence_report(
        ...
        **kwargs,
        ...
        analysis = {
            "report_model": (
                "evidence-report-v2+laya"
                if verification.analysis["verification_model"]["name"] == "laya-stance-v1"
                else "evidence-report-v2"
            ),
