...
Install with::
    ...
    pip install "zero-api-key-web-search[laya]"

Environment variables
----------------------

===========================  ==============================================
``ZERO_SEARCH_LAYA_MODEL``     HF repo id (default ``convaiinnovations/laya``)
``ZERO_SEARCH_LAYA_SUBFOLDER`` ``multilingual`` / ``typed-decisions`` / unset
``ZERO_SEARCH_LAYA_DEVICE``    ``cuda`` / ``mps`` / ``cpu`` (default: auto)
...
class LayaBackend:
    ...
    :meth:`predict`, never at import time, so ``import zero_api_key_web_search`` stays
    ...
    def __init__(
        ...
        self.model_id = model_id or os.environ.get("ZERO_SEARCH_LAYA_MODEL", DEFAULT_LAYA_MODEL)
        self.subfolder = subfolder or os.environ.get("ZERO_SEARCH_LAYA_SUBFOLDER") or None
        self.device = device or os.environ.get("ZERO_SEARCH_LAYA_DEVICE") or None
        self.token = token
        ...
        with self._lock:
            ...
            try:
                ...
                self._load_error = (
                    ...
                    'Install it with: pip install "zero-api-key-web-search[laya]"'
                )
...
def add_laya_args(parser) -> None:
    ...
    group.add_argument(
        "--laya-model",
        default=None,
        help=f"HuggingFace repo id (default: {DEFAULT_LAYA_MODEL}, or $ZERO_SEARCH_LAYA_MODEL).",
    )
    group.add_argument(
        "--laya-subfolder",
        default=None,
