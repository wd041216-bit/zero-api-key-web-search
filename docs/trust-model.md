...
## Laya Stance Model (`laya-stance-v1`, optional)

When verification runs with `verifier="laya"` (CLI `--laya`), per-source stance
classification is delegated to the Laya decision model
(`convaiinnovations/laya`, Apache-2.0, local inference): two yes/no questions
per source ("does this support the claim?" / "does it state or imply the claim
is false?") answered with probabilities in a single forward pass.

What changes vs. the lexical model:

- Stance is probabilistic and semantic rather than keyword-overlap based, so
  paraphrases, negation scope, and mismatched numbers are no longer invisible.
- Evidence strength blends `0.55 * stance + 0.35 * quality + 0.10 * freshness`.
- The lexical payload is kept as a sidecar for comparability.

What does not change:

- No logical proof or fact-level parsing; probabilities are inputs, not proofs.
- **The probabilities are not domain-calibrated.** Upstream measured ECE 0.466
  before and 0.081 only after per-domain temperature refitting. Thresholds
  (`--laya-threshold`, default 0.5) are reasonable defaults, not calibrated
  operating points — tune against a small labeled set from your domain.
- Base checkpoints are weak zero-shot on specialized decision workflows
  (upstream typed-decisions: 0.362 zero-shot vs 0.766 fine-tuned). For
  high-stakes domains, fine-tune the open checkpoint and load it with
  `--laya-model` / `--laya-subfolder`.
- Page-aware (`--deep`) rescoring still uses the lexical path.

Every failure degrades to the lexical model with a recorded note; a missing
`laya` package or checkpoint never removes evidence from a report.
